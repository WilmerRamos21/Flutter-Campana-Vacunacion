...
class CreateVaccinatorScreen extends StatefulWidget {
  const CreateVaccinatorScreen({super.key});

  @override
  State<CreateVaccinatorScreen> createState() => _CreateVaccinatorScreenState();
}

class _CreateVaccinatorScreenState extends State<CreateVaccinatorScreen> {
  ...
  static const String _role = 'vacunador';

  final List<int> _sectoresPermitidos = [1, 2, 3, 4, 5];

  Future<void> _createVaccinator() async {
    if (!_formKey.currentState!.validate()) return;

    if (_selectedSectorId == null) {
      _showMessage('Seleccione el sector del vacunador.');
      return;
    }

    if (_role != 'vacunador') {
      _showMessage('Solo se permite crear usuarios con rol vacunador.');
      return;
    }
    ...
    try {
      ...
      _showMessage('Vacunador creado correctamente.');
      _clearForm();
    } catch (error) {
      _showMessage('Error al crear vacunador: $error');
    } finally {
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: FilledButton.icon(
                    onPressed: _isLoading ? null : _createVaccinator,
